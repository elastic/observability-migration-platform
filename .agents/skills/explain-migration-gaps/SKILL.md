...
> **Exception — Grafana `stat`/`gauge` status grids:** a dashboard can report 19/19 ✓ with `semantic_gate: Green` and still collapse a multi-series Grafana stat (e.g. Target health `up`) into one `MAX()`/`LAST()` scalar. Legend placeholders such as `{{job}}` are display aliases, not series identity — they are **not** turned into `BY` dimensions (that would widen a scalar `sum(rate(...))` and still merge instances that share a job). Ungrouped selectors migrate **with a warning**. If the user points at a status grid that is still one number in Kibana, do **not** skip it because status is `migrated` — compare source tiles vs the emitted query.
...
| Stat/gauge status grid / `{{job}}` / ungrouped `up` | Grafana stat/gauge tiles are one-per-series. Translation preserves the source PromQL grouping/cardinality; `legendFormat` placeholders stay display aliases (not `BY` dimensions). Ungrouped `up` that Grafana still fans out migrates **with a warning** | Compare source tiles vs Kibana. A 19/19 ✓ with zero reasons should no longer hide this; if you still see one scalar for a multi-series Grafana stat, rebuild as a grouped metric or table. |
