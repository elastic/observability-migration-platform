...
| `--esql-index` | Grafana | The index / data stream **written into the generated ES\|QL queries and inspected during target schema discovery** | Defaults to `--data-view` when unset. Override it (with `--es-url`) when the queries should read from, and fields should be discovered from, a specific data stream — required for Prometheus fidelity. Grafana-only today; Datadog controls its metric query target through `--data-view` / the active `--field-profile` instead. See [Target index flags](#target-index-flags-data-view-vs-esql-index). |
...
| `--esql-index` | falls back to `--data-view` when unset | The **index / data stream pattern written into the generated ES\|QL queries** (the `FROM` target the queries actually read) **and the target of schema discovery** — the index the tool inspects (with `--es-url`) to learn your field layout. |

In code the Grafana schema resolver is constructed with
`index_pattern=args.esql_index or args.data_view`, so when you omit
`--esql-index` it inherits `--data-view` for both query emission and field
...
panels to the broad `metrics-*` data view while reading from the
`metrics-alloy.prometheus-default` stream:
...
only changes the emitted-query `FROM` target; the run falls back to OTel field
