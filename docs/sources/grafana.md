...
- Some PromQL families still degrade to `not_feasible` or manual review, especially subqueries, `bottomk`/`count_values`, known-wrong histogram field types (for example `aggregate_metric_double`), generic `sum(A/B)` that is not a `_sum`/`_count` pair, `__name__` introspection, and multi-branch join/or cases that cannot fuse.
