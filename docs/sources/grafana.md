...
- Preflight `datasource_audit.non_migratable` resolves Grafana datasource **template variables** from `templating.list` (`type: datasource`, `query` is the type filter, e.g. `influxdb`). An unresolved `$datasource` / `${DS_*}` is recorded in `unresolved_datasource_variables` and treated as a blocker — do not read `non_migratable: []` as a clean bill when those fields are populated.
