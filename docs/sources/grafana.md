...
| Lint / compile / layout | `targets/kibana/compile.py` | Only with `--compile`/`--legacy-import`: render each dashboard's kb-dashboard YAML from its IR into a scratch directory, lint it, compile NDJSON, validate compiled layout, then delete the scratch directory |
...
- `topk(k, expr)` and `bottomk(k, expr)` translate to an ES|QL `SORT value DESC/ASC | LIMIT k` over the latest bucket per group. A warning is emitted; the result is an approximation when the source had many buckets. Ungrouped forms collapse to a single-series ranking with a `preferred_group_labels` hint. When a `drop_rate` metric_map entry remaps the inner metric to a gauge, both `topk`/`bottomk` and scaled-rate expressions (`sum(rate(...)) * scalar`) switch to `FROM` source automatically — using `TS` source on a gauge field would query the wrong index and inflate averages.
