...
`changes()` on the restart counter is `RATE` over the latest 30-minute
bucket, times 1800 seconds (ES|QL `DELTA` rejects counter fields). Deployment, StatefulSet, job, and PVC
tables group by the object the Grafana legend named. HPAs sitting on their
min or max are tables. The CPU capacity chart names capacity, allocatable,
...
as available bytes divided by one day of used-byte growth, and replaces the
...
stays a single number. Cluster, namespace, and volume controls stay on the
queries, along with the kubelet job. The scrape `metrics_path` selector is
...
Grafana's `min`/`max` of a per-instance sum is one node; Kibana sums the
...
- A literal match-all regex matcher (`{hpa=~".*"}`) is not a variable. It also
  matches series where the label is absent, so omitting it changes nothing and
  it no longer raises *Dropped variable-driven label filters during migration*
  (issue #375) — that warning had pointed operators at a filter loss that never
  happened.
...
- **Element-wise arithmetic between different metric names uses ES|QL, not
  ...
  shared labels. Panels of that shape are therefore translated to the ES|QL
  per-key form (`STATS a = …, b = … BY <labels>, TBUCKET | EVAL result = a / b`)
  and reported as **migrated with warnings**, carrying the same-bucket
  approximation warning plus a panel note naming the reason. Without a
  `legendFormat` / `by()` the fallback is a bucket-level ratio of aggregates,
  not a per-series join. This covers the operands wrapped in a function too —
  `rate(A[5m]) / rate(B[5m])` keeps the metric name on this target even though
  Prometheus drops it. Ratios whose operands are *aggregated* (`sum(A) / sum(B)`,
  `avg by (ns) (A) / avg by (ns) (B)`), mixed aggregation against a bare metric
  (`sum(A) / B`, which Elasticsearch rejects with a 400 rather than returning
  empty), ratios of the *same* metric under two selectors
  ...
  Alert rules of the same distinct-metric shape stay automated through ES|QL
  when the translator can emit a query; untranslatable functions (`changes()`,
  `absent()`, `predict_linear()`) and control-bound matchers
  (`{namespace=~"$namespace"}`, which an alert has nothing to bind) go
  `manual_required` rather than advertising a query-less rule. `{__name__=~"..."}`
...
- An aggregation **wrapping** a binary operator (`agg(A op B)`) is only feasible when a rewrite can prove the operator survives, because PromQL evaluates `A op B` per matching series pair — matching on the operands' full label set — before the aggregation reduces the survivors, and ES|QL has no equivalent stage. The rewrites are: `sum(A ± B)` push-down (`sum` is linear), scalar hoisting (`max(A * 8)`, `avg(A / 100)`), the histogram mean idiom above, and the co-located per-document renderer (`count(A + B)` → `COUNT((A + B))`, for `+ - * /` over operands that share a label set). When live field co-occurrence proves those operands never share a document (Prometheus remote_write and similar one-sample-per-document layouts), the same shape is instead rendered as independent same-bucket `STATS` aggregations combined with `EVAL`, then the outer aggregation — per-document `WHERE A AND B` would match no rows and ship an empty panel. Offline, or when co-occurrence is unknown, the per-document renderer is kept. Everything else stays `not_feasible` with a warning naming the operator and the aggregation: set operators `and` / `unless` (`count(A and B >= threshold)` — issue #377), comparisons between two series (`count(A < B)`; compare against a scalar threshold instead), and arithmetic the renderer does not cover (`%`, `^`, `atan2`, or operands that carry their own `by()` reduction). The refusal is an allowlist of operators that are handed on rather than a list of operators that are refused, so an operator nobody enumerated fails closed instead of silently reaching the fallback. Previously these fell through to the generic single-metric fallback, which rebuilt `agg(<first metric>)` and shipped a plausible but wrong number as `migrated_with_warnings`.
- A comparison operator (`==`, `!=`, `>`, `<`, `>=`, `<=`) is a **filter**, not arithmetic, and translates by which form it takes. Against a scalar threshold it becomes a `| WHERE` clause, before the `STATS` when an aggregation wraps it (`sum(phase == 1)`) and after it otherwise (`node_load1 > 0.5`). Between two vectors it becomes `| EVAL computed_value = CASE(<lhs> <op> <rhs>, <value>, NULL)` plus `| WHERE computed_value IS NOT NULL`, so the elements that satisfy it keep the *vector* operand's own value (the left one, or the right one when the scalar is on the left) and the rest are dropped, as PromQL does. Previously this emitted `(<lhs> <op> <rhs>)`, labelling every series `true`/`false` instead of filtering (issue #375). The explicit `bool` modifier asks for the opposite and still renders `CASE(cond, 1, 0)`, NULL-guarded to `CASE(cond, 1, NULL)` when the indicator is a divisor. When a `bool` comparison's own operand is a bare comparison (`(a > b) > bool 0`), the indicator is guarded on that operand too: the inner filter leaves NULL for an element it dropped, and a NULL condition would otherwise fall through to `CASE`'s default and report it as a real `0`. A `0` that came from a *surviving* element is still reported. When both operands are filters the guard requires both to have survived, and the same drop carries through when such an indicator is used as a divisor. The residual approximation is the same one the same-bucket arithmetic caveat already names: PromQL matches operands on their full label set per instant, ES|QL per shared time bucket and surviving `BY` dimensions, so a panel whose series collapsed to one line compares two aggregates rather than per-series pairs. An aggregation *wrapping* a two-series comparison (`count(A < B)`) stays `not_feasible` — see the bullet above.
- `agg(A or B)` is decided one stage later, during classification, because `or` carries two reductions that need live field capabilities the parser does not have: the same-metric range-window fallback (`max_over_time(M[$interval]) or max_over_time(M[5m])`, `rate(M[$interval]) or irate(M[5m])`) and the drop of an operand whose metric is **provably absent** from the target. When either reduction elects a single surviving operand the panel translates as before, and the dropped operand is now named in a warning — the same disclosure the bare (unaggregated) chain already emitted. When neither applies, the panel is `not_feasible` (issue #434). `or` is a set *union*, so an aggregation cannot be distributed over it (`agg(A or B)` is not `agg(A) or agg(B)`), which is why the `COALESCE` / unified `WHERE ... OR` rewrites that keep both operands in the bare form are unavailable under an aggregation. Before this, `count(node_a or node_b)` emitted `COUNT(node_a)` as clean `migrated` with no warning at all, while the bare `node_a or node_b` kept both operands — the wrapper inverted the verdict on the same expression. The refusal covers `by()` grouping, longer `or` chains, every outer aggregation, enclosing wrappers such as `clamp_max(...)`, nested aggregations (`sum(count(A or B))`, `max(sum(A or B))`), `label_replace`/`label_join` around an inner union, comparisons against a scalar (`avg(A or B) > 5`), and `topk`/`bottomk` over a union (`topk(3, A or B)`). Offline (no `--es-url` / no reachable field capabilities) nothing is disprovable, so a cross-metric fallback chain refuses rather than silently reporting only its left operand; re-run with target field capabilities reachable to let the absent-operand reduction apply.
